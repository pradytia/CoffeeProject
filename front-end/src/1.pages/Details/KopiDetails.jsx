...
class KopiDetails extends Component {
    ...
    getDataKopi = () => {
        Axios.get(urlApi + '/product/getkopi/' + this.props.match.params.id)
        .then(res => {
            this.setState({ kopiDetails :  res.data })
        })
        .catch(err => {
            console.log(err)
        })
    ...
    renderKopi = () => {
        return this.state.kopiDetails.map(val=>{
            return(
...
export default connect(mapStateToProps)(KopiDetails);